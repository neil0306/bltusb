...
# Pass HELPER_DIR as a positional arg (not interpolated into the code), so a path
# containing a quote/space cannot break out of the `bash -c` string.
sudo -u "$BUILD_USER" bash -c 'cd -- "$1" && exec swift build -c release -Xswiftc -DBLTUSB_SELFHOSTED' bash "$HELPER_DIR" >/dev/null
BIN_DIR="$(sudo -u "$BUILD_USER" bash -c 'cd -- "$1" && exec swift build -c release -Xswiftc -DBLTUSB_SELFHOSTED --show-bin-path' bash "$HELPER_DIR")"
