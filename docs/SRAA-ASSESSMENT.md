...
### 13.2 `vm build|update|status` subcommand — independent audit (codex + Fable 5 + Sonnet validation)

The `vm` subcommand (self‑built, digest‑pinned, updatable microVM) was independently audited by two auditors (codex, Fable 5), PoC‑validated (Sonnet), fixed (Opus), and re‑audited. **No backdoor, exfiltration, or command injection was found; the BitLocker passphrase never reaches `vm`.** Confirmed‑shipping findings were all **fixed**:
- **Kernel/modules supply‑chain (was "unpinned URLs"):** the toml `kernel.*_url` are declarative only — the guest boots the **sha256‑pinned Homebrew‑formula** `libexec/Image`, `Image‑4K`, `lib/modules.squashfs`. `_vm_verify_kernel()` now sha256‑verifies those **local** artifacts before every real build and **fails closed** on a mismatch **or a missing expected artifact** when the anylinuxfs prefix is resolvable; it reverts to the formula pin (warn) only when genuinely uncheckable (non‑standard install / no `shasum`).
- **TOML write:** `_vm_write_toml()` refuses a symlinked destination **and** a symlinked `.bak`, writes a same‑directory temp, validates it, and `mv -f` (atomic rename) — closing the demonstrated symlink‑overwrite + partial‑write PoC.
- **`forget`:** the device argument is now `canonical_device`‑normalized and `^/dev/disk[0-9]+s[0-9]+$`‑anchored before any raw read.
- **osascript:** volume labels have C0/DEL bytes folded to spaces (LF/CR/TAB can't wedge the dialog).

**Accepted residuals (non‑privilege‑escalation — `vm`/`forget` run as the *invoking user*, not root, so these cross no privilege boundary):** the verify‑then‑use gap on the user‑owned Homebrew kernel artifacts and the write‑path TOCTOU on the user's own toml are the **same single‑user, user‑writable‑Homebrew baseline as O‑04/M3** — their real fix (FD‑pinned verification, root‑owned artifacts) is the **Mode B / patched‑anylinuxfs** track, not a bash‑level control. The remaining audit findings (helper backend‑verify stub, installer not wiring `ANYLINUXFS_ROOT_STORE`, caller‑auth, passphrase FD channel) are **Mode B integration items**, tracked for the signed‑helper phase — they do not affect the shipping Mode 0 + `vm` surface.
