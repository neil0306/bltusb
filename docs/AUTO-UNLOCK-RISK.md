...
- **Deliver the volume passphrase to the backend without it transiting the process environment (R-env).** anylinuxfs exposes **no** stdin/pipe/file-descriptor channel for the BitLocker passphrase today, so the only supported way to hand it to the privileged mount is the `ALFS_PASSPHRASE` environment variable (`ALFS_PASSPHRASE="$pass" sudo --preserve-env=ALFS_PASSPHRASE anylinuxfs mount …`). We keep the secret **out of argv, out of any logged command, and off disk** — it is a single inline assignment, never `export`ed, so it lives only in the environment of the `sudo` child and the `anylinuxfs` grandchild, and only for that one mount's lifetime. The honest residual: for that brief lifetime the passphrase is **transiently observable in the process environment of the root `anylinuxfs`/`sudo` process** by anything that can read a root process's environment (env-aware `ps -E`, `/proc`-style environ reads — on macOS the kernel already redacts another user's process environment from non-root callers, so this is a **root/local-privileged** observation, not a cross-user leak). It is **never written to a log or to disk in cleartext**. The future hardening is a **file-descriptor / pipe channel** to the backend (delivered under the Phase-2 signed helper, where the daemon reads the secret over an authenticated FD and zeroes it after use — see `SRAA-ASSESSMENT.md`), which removes the environment residual entirely. Until that upstream/helper channel exists, R-env is an **inherent anylinuxfs-design residual, accepted for Phase-0 personal-dev use**.
